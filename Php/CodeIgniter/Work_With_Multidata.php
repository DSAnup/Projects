...
<?php
  //Controller
    public function add_keyproduct(){
        $userID = $this->session->userdata('userID');
        if (isset($userID)) {
        $data= $this->input->post();
        // echo "<pre>";
        // print_r($data);
        foreach ($data['keyName'] as $key => $value) {
            $d['keyName'] = $data['keyName'][$key];
            $d['pId'] = $data['pId'];
            $this->Rest_model->SaveData('keyproduct', $d);
        }
        
        redirect(base_url().'Admin_panel/keyproduct');
        }else{
