...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $image_file = file_get_contents('http://localhost:8080/tailors/uploads/logo.png');
        $this->Image('@'.$image_file, 10, 10, 15, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
        // Set font
        $this->SetFont('helvetica', 'B', 20);
        // Title
        $h = '<h1>HEADER</h1>';
        $this->WriteHTMLCell(0,0,'','',$h,0,1,0,true,'C',true);
    }
    ...
    public function Footer() {
        ...
        $this->WriteHTMLCell(0,0,'','',$h,'LRTB',1,0,true,'L',true);
        // $this->Ln(2);
        $this->WriteHTMLCell(0,0,'','','Page '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(),0,1,0,true,'R',true);
...
$h = "hello";
foreach($allc as $c){
$pdf->write1DBarcode($c->name, 'C39', '', '', '', 18, 0.4, $style, 'N');
}
$pdf->Ln();
...
foreach($allc as $c){
$pdf->write1DBarcode($c->id, 'S25', '', '', '', 18, 0.4, $style, 'N');
}
$pdf->Ln();
