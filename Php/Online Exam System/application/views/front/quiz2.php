...
<div class="quiz">
	<div class="">
		...
		<form id="form1" action="<?=base_url().'Front/check_answer'?>" method="post">
			...
			<input type="text" name="enroll_id" value="<?= $rand ;?>">
			<?php
			$i=0;
			foreach($quiz as $q){
				$i++;
	...
	<div class="course_name">
		<ul>
			...
			foreach ($subject as $k) {
				...
				if(empty($qq)){
				echo "<li><a href='".base_url()."Front/take_quiz2/".$k->id."'><i class='fa fa-arrow-right' aria-hidden='true'></i>".$k->name."</a></li>";
				}else{
				echo "<li>".$k->name."</li>";
			}
			}
...
<script type="text/javascript">
	...
	$(window).blur(function(){
		...
		i++;
		alert("Don't try to access anything before completing exam");
		if(i == 3){
			alert("Final warning! if you try again you will be eligible for this exam");
		}
		if(i > 3){
